					...
					<div v-for="member in members.data" class="col-lg-4 col-md-6 d-flex align-items-stretch mb-3" data-aos="zoom-in" data-aos-delay="100">
						<div :class="(member.id % 2 == 0) ? 'icon-box iconbox-blue' : 'icon-box iconbox-red'">
							<div class="icon">
								...
								<img v-if="member.avatar" :src="`http://localhost:8000/storage/${member.avatar}`" class="img-fluid img-responsive" width="150">
...
<script>
	...
	export default {
		...
		methods: {
			listMembers(page){
				this.$axios
				.get(`http://localhost:8000/api/evoush/member-list?page=${page}`)
				.then(res => {
					// console.log(res)
					return res.data
				})
